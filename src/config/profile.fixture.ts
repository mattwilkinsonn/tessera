// FROZEN TEST FIXTURE — Matt's real layout, NOT the shipped default.
//
// This is the byte-identical port of the original layout profile, kept only so
// the port-fidelity suite (`profile.test.ts`) and the engine unit tests bind
// against stable, known data. The binary never loads this file; the shipped
// default lives in `profile.ts` and a user's real profile is resolved at
// runtime from `~/.config/tessera/profile.ts` (or `$TESSERA_PROFILE`).
//
// Pure data, no logic. `satisfies Profile` gives the compile-time validation
// the bash file's shellcheck-disabled data block could not.
//
